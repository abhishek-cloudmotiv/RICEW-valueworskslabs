...
const ServiceForm = ({
  ...
  const getNextServiceId = async () => {
    try {
      ...
      const response = await fetch('https://vs7ws23ybl.execute-api.ap-south-1.amazonaws.com/rice/serviceform/next-id', {
        headers: headers
      });
  ...
  const loadServiceData = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://vs7ws23ybl.execute-api.ap-south-1.amazonaws.com/rice/serviceform/getDataByPortfolio?portfolioId=${portfolioDisplayId}&project_id=${projectId}`, {
        headers: headers
      });
      ...
      if (response.ok) {
        ...
        if (result && result.data && Array.isArray(result.data) && result.data.length > 0) {
          // Filter by active records (backend already filters by portfolio display ID)
          ...
          const transformedData = filteredRecords.map((item, index) => ({
            id: index + 1,
            serviceId: DOMPurify.sanitize(String(item.Service_DisplayID || `SV${index + 1}`).trim(), { ALLOWED_TAGS: [] }),
            serviceActualId: DOMPurify.sanitize(String(item.Service_Form_ID || '').trim(), { ALLOWED_TAGS: [] }),
            name: DOMPurify.sanitize(String(item.Service_Name || '').trim(), { ALLOWED_TAGS: [] }),
          ...
          // Sort by Service_Form_ID (primary key) - ascending order
          transformedData.sort((a, b) => {
            const idA = parseInt(a.serviceId.replace(/\D/g, '')) || 0;
            const idB = parseInt(b.serviceId.replace(/\D/g, '')) || 0;
            return idA - idB;
          });
  ...
  const handleAddService = async () => {
    ...
    if (editingItem !== null) {
      ...
      if (editingItemData && editingItemData.isSaved) {
        ...
        showConfirmation(
          'You have unsaved changes. Do you want to discard them and add a new service?',
          async () => {
  ...
  const addNewServiceRow = async () => {
    // Get the next service ID from API for display purpose
    const nextSvId = await getNextServiceId();
    if (!nextSvId) {
      setErrorMessage('Failed to generate service ID. Please try again.');
  ...
  const handleSaveEdit = async (id) => {
    try {
      ...
      // Service Code is now auto-generated by backend, no frontend validation needed
      ...
      if (isUpdate) {
        ...
        payload = {
          Service_Form_ID: editValues.serviceActualId,
          Service_Name: editValues.name,
        ...
        endpoint = 'https://vs7ws23ybl.execute-api.ap-south-1.amazonaws.com/rice/serviceform/updateRecord';
      } else {
        // Create new record
        payload = {
          Service_Name: editValues.name,
        ...
        endpoint = 'https://vs7ws23ybl.execute-api.ap-south-1.amazonaws.com/rice/serviceform/postRecord';
      }
      ...
      if (response.ok) {
        setSuccessMessage(isUpdate ? 'Service Line updated successfully!' : 'Service Line saved successfully!');
        ...
        setData(data.map(item =>
          item.id === id
            ? {
              ...
              lastUpdatedBy: userId,
              lastUpdatedDate: currentDate,
              ...(!isUpdate && result.data ? {
                serviceActualId: result.data.id || result.data.Service_Form_ID,
                serviceId: result.data.Service_DisplayID,
        ...
        setErrorMessage(result.error || `Duplicate Entry: Service "${editValues.name}" already exists in this hierarchy.`);
        setShowErrorMessage(true);
        setTimeout(() => setShowErrorMessage(false), 5000);
      } else {
        setErrorMessage(result.error || 'Failed to save or update service');
        setShowErrorMessage(true);
        setTimeout(() => setShowErrorMessage(false), 3000);
      }
    } catch (error) {
      console.error('Error saving or updating service:', error);
      handleAuthError(error.message);
    }
  };
        ...
        showConfirmation(
          ...
          async () => {
            try {
              ...
              const payload = {
                Service_Form_ID: rowToDelete.serviceActualId,
                project_id: projectId,
                lastupdatedby: userId
              };
              ...
              const response = await fetch('https://vs7ws23ybl.execute-api.ap-south-1.amazonaws.com/rice/serviceform/deleteRecord', {
                method: 'DELETE',
                headers: headers,
                body: JSON.stringify(payload)
              });
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div>
            <h1 style={{ margin: '0 0 8px 0', color: '#333', fontSize: '24px', fontWeight: '600' }}>Service Line Form</h1>
            <div style={{ display: 'flex', gap: '80px', alignItems: 'center', marginBottom: '4px' }}>
              ...
              <h2 style={{ margin: '0', color: '#333', fontSize: '18px' }}>Portfolio: {portfolioName}</h2>
            </div>
            <p style={{ margin: '0', color: '#666', fontSize: '14px' }}>Portfolio ID: {portfolioDisplayId}</p>
          </div>
                ...
                <div style={{ color: '#444', fontSize: '14px', lineHeight: '1.6' }}>
                  ...
                  <div style={{ marginBottom: '16px' }}>
                    ...
                    <p style={{ margin: '6px 0 0 0', color: '#555' }}>
                      The <strong>Service Line Form</strong> manages the service lines that belong to a specific Portfolio. A Service Line represents a granular delivery unit within a portfolio (e.g., Data Migration, Integration Development, UAT Support) and is the lowest level of the organization hierarchy before individual resources are assigned.
                    </p>
                  </div>
                  ...
                  <div style={{ marginBottom: '16px' }}>
                    ...
                    <p style={{ margin: '6px 0 0 0', color: '#555' }}>
                      Service Lines are the fourth level of the organization hierarchy: <strong>Organization → Business Line → Portfolio → Service Line</strong>. They are used to define the specific work streams that resources are assigned to, and are referenced in Rate Cards and Resource Rosters for accurate cost tracking and staffing.
                    </p>
                  </div>
                  ...
                  <div style={{ marginBottom: '16px' }}>
                    ...
                    <ul style={{ margin: '6px 0 0 0', paddingLeft: '18px', color: '#555' }}>
                      <li><strong>Service ID</strong> — System-generated unique identifier for the service line.</li>
                      <li><strong>Code</strong> — Auto-generated short code derived from the Service Line Name. Read-only; updated by the backend when the name changes.</li>
                      <li><strong>Service Line Name</strong> — The name of the specific delivery work stream (required, max 100 characters).</li>
                      <li><strong>Description</strong> — A brief explanation of what this service line covers (required, max 200 characters).</li>
                      <li><strong>Comments</strong> — Optional notes about the service line (max 240 characters).</li>
                      <li><strong>Actions</strong> — Edit or Delete the service line via the ⋮ menu.</li>
                    </ul>
                  </div>
                  ...
                  <div style={{ marginBottom: '16px' }}>
                    ...
                    <ul style={{ margin: '6px 0 0 0', paddingLeft: '18px', color: '#555' }}>
                      <li>Click <strong>Add Service Line</strong> to create a new row. Enter the Name and Description, then click the <strong>Save (✓)</strong> icon to save.</li>
                      <li>Use the <strong>⋮ Actions</strong> menu on a saved row to <strong>Edit</strong> or <strong>Delete</strong> it.</li>
                  ...
                  <div style={{ marginBottom: '4px' }}>
                    ...
                    <ul style={{ margin: '6px 0 0 0', paddingLeft: '18px', color: '#555' }}>
                      <li><strong>Service Line Name</strong> and <strong>Description</strong> are required — saving will fail without them.</li>
                      <li>The <strong>Code</strong> is auto-generated by the backend and cannot be edited manually.</li>
                      <li>Duplicate service line names within the same portfolio hierarchy are not allowed — a conflict error will appear if a duplicate is detected.</li>
                      <li>Only one row can be in edit mode at a time. You will be prompted to discard changes before switching rows.</li>
                      <li>Service lines are scoped to the Portfolio shown in the header — they will not appear under other portfolios.</li>
                    </ul>
                  </div>
        ...
        <div style={{ flex: 1, overflow: 'auto', position: 'relative' }}>
          <div className="service-table-container" style={{ maxHeight: '500px', overflowY: 'auto', overflow: 'visible' }}>
            <table className="config-table" style={{ fontSize: '15px' }}>
              <thead>
                <tr>
                  <th style={{ padding: '8px 12px', fontSize: '16px', width: '12%' }}>Service ID</th>
                  <th style={{ padding: '8px 12px', fontSize: '16px', width: '12%' }}>Code</th>
                  <th style={{ padding: '8px 12px', fontSize: '16px', width: '20%' }}>Service Line Name</th>
                      ...
                      <td style={{ padding: '6px 12px', verticalAlign: 'top' }}>
                        <div style={{ paddingTop: editingItem === item.id ? '18px' : '0' }}>
                          {editingItem === item.id && item.isSaved ? (
                            <span style={{ fontWeight: '500', color: '#374151' }}>{item.serviceId}</span>
                          ) : (
