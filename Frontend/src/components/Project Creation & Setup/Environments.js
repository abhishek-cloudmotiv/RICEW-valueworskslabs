...
const Environments = ({ onClose, selectedProject, onBackToLanding, onLogout, setUnsavedChangesChecker }) => {
  ...
  // Constants
  const API_BASE_URL = 'https://35j96p30rd.execute-api.ap-south-1.amazonaws.com/New/ricew/environment';
  ...
  // Register unsaved-changes checker once — reads from refs so it always sees latest values
  useEffect(() => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const sortedRecords = [...records].sort((a, b) => {
          const idA = parseInt(a.Environment_id) || 0;
          const idB = parseInt(b.Environment_id) || 0;
          return idA - idB;
        });

        const mappedData = sortedRecords.map(item => ({
          id: DOMPurify.sanitize(String(item.Environment_id || '').trim(), { ALLOWED_TAGS: [] }),
          environmentCode: DOMPurify.sanitize(String(item.Environment_Code || '').trim(), { ALLOWED_TAGS: [] }),
          environmentName: DOMPurify.sanitize(String(item.Environment_Name || '').trim(), { ALLOWED_TAGS: [] }),
          purpose: DOMPurify.sanitize(String(item.Environment_Purpose || '').trim(), { ALLOWED_TAGS: [] }),
        ...
        console.error('Failed to fetch environments data');
        setData([]);
      }
    } catch (error) {
  ...
  const handleAddEnvironment = () => {
    ...
    if (editingItem !== null) {
      ...
      if (editingItemData && editingItemData.isSaved) {
        showConfirmation(
          'You have unsaved changes. Do you want to discard them and add a new environment?',
          () => {
            setEditingItem(null);
            setEditValues({});
            setValidationErrors({});
  ...
  const handleSave = async (id) => {
    ...
    if (!editValues.environmentCode || editValues.environmentCode.trim() === '') {
      fieldsWithErrors.environmentCode = true;
    }
    if (!editValues.environmentName || editValues.environmentName.trim() === '') {
      fieldsWithErrors.environmentName = true;
    }
    ...
    if (Object.keys(fieldsWithErrors).length > 0) {
      ...
      setErrorMessage('Environment Code and Environment Name are required.');
    ...
    try {
      ...
      if (hasNewRow) {
        ...
        payload = {
          ...
          Environment_Code: DOMPurify.sanitize(String(editValues.environmentCode || '').trim(), { ALLOWED_TAGS: [] }),
          Environment_Name: DOMPurify.sanitize(String(editValues.environmentName || '').trim(), { ALLOWED_TAGS: [] }),
          Environment_Purpose: DOMPurify.sanitize(String(editValues.purpose || '').trim(), { ALLOWED_TAGS: [] }),
          created_by: DOMPurify.sanitize(String(localStorage.getItem('user_id') || '1').trim(), { ALLOWED_TAGS: [] })
        };
      } else {
        endpoint = `${API_BASE_URL}/update`;
        payload = {
          Environment_id: DOMPurify.sanitize(String(id || '').trim(), { ALLOWED_TAGS: [] }),
          Environment_Code: DOMPurify.sanitize(String(editValues.environmentCode || '').trim(), { ALLOWED_TAGS: [] }),
          Environment_Name: DOMPurify.sanitize(String(editValues.environmentName || '').trim(), { ALLOWED_TAGS: [] }),
          Environment_Purpose: DOMPurify.sanitize(String(editValues.purpose || '').trim(), { ALLOWED_TAGS: [] }),
      ...
      if (response.ok) {
        setSuccessMessage(hasNewRow ? 'Environment added successfully!' : 'Environment updated successfully!');
        ...
        throw new Error(errData.message || errData.error || (hasNewRow ? 'Failed to create environment' : 'Failed to update environment'));
      }
    } catch (error) {
      console.error('Error saving environment:', error);
      setErrorMessage(error.message || 'Failed to save environment');
      setShowErrorMessage(true);
      setTimeout(() => setShowErrorMessage(false), 3000);
    } finally {
      setLoading(false);
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const errData = await response.json().catch(() => ({}));
        throw new Error(errData.message || errData.error || 'Failed to delete environment');
      }
    } catch (error) {
      console.error('Error deleting environment:', error);
      setErrorMessage(error.message || 'Failed to delete environment');
      setShowErrorMessage(true);
      setTimeout(() => setShowErrorMessage(false), 3000);
    } finally {
      setLoading(false);
  ...
  return (
    <div className="config-main" style={{ margin: '2rem 2rem 0 2rem', paddingBottom: '0.1rem' }}>
      ...
      <div className="config-header" style={{ marginTop: '0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <h2>Project Instances / Environment</h2>
        <button
              ...
              <div style={{ color: '#444', fontSize: '14px', lineHeight: '1.6' }}>
                ...
                <div style={{ marginBottom: '16px' }}>
                  ...
                  <p style={{ margin: '6px 0 0 0', color: '#555' }}>
                    The <strong>Project Instances / Environment</strong> page manages the technical environments (also called instances or landscapes) used throughout the ERP project lifecycle. Each environment represents a distinct system setup where specific activities — such as development, testing, or production deployment — take place (e.g., DEV, QA, UAT, PROD).
                  </p>
                </div>
                ...
                <div style={{ marginBottom: '16px' }}>
                  ...
                  <p style={{ margin: '6px 0 0 0', color: '#555' }}>
                    Clearly defining environments ensures that RICEW work items, testing cycles, and deployment activities are tracked against the correct system landscape. It supports change management, testing governance, and release planning by providing a shared reference for all project teams on which environments exist and what each one is used for.
                  </p>
                </div>
                ...
                <div style={{ marginBottom: '16px' }}>
                  ...
                  <ul style={{ margin: '6px 0 0 0', paddingLeft: '18px', color: '#555' }}>
                    <li><strong>Environment Code</strong> — A short uppercase code that identifies the environment (e.g., DEV, QA, UAT, PROD). Max 30 characters, letters, numbers, and hyphens only.</li>
                    <li><strong>Environment Name</strong> — The full descriptive name of the environment (e.g., Development, Quality Assurance, Production). Required, max 100 characters.</li>
                    <li><strong>Purpose</strong> — An optional description of what activities this environment supports. Max 240 characters.</li>
                    <li><strong>Actions</strong> — Edit or Delete an environment via the ⋮ menu.</li>
                  </ul>
                </div>
                ...
                <div style={{ marginBottom: '16px' }}>
                  ...
                  <ul style={{ margin: '6px 0 0 0', paddingLeft: '18px', color: '#555' }}>
                    <li>Click <strong>Add Environment</strong> to create a new row. Fill in the Environment Code and Environment Name, then click <strong>Save (✓)</strong> to commit.</li>
                    <li>Use the <strong>⋮ Actions</strong> menu to <strong>Edit</strong> or <strong>Delete</strong> a saved environment.</li>
                ...
                <div style={{ marginBottom: '4px' }}>
                  ...
                  <ul style={{ margin: '6px 0 0 0', paddingLeft: '18px', color: '#555' }}>
                    <li><strong>Environment Code</strong> and <strong>Environment Name</strong> are required — saving will fail without them.</li>
                    <li>Environment Codes are automatically converted to uppercase and allow only letters, numbers, and hyphens.</li>
                    <li>A project must be selected before environments can be loaded or added.</li>
                  </ul>
                </div>
        ...
        <table className="config-table" style={{ fontSize: '15px', width: '100%', borderCollapse: 'collapse', tableLayout: 'fixed' }}>
          <thead>
            <tr style={{ backgroundColor: '#f8f9fa', borderBottom: '2px solid #dee2e6' }}>
              <th style={{ padding: '12px', textAlign: 'left', fontWeight: '600', width: '20%' }}>Environment Code</th>
              <th style={{ padding: '12px', textAlign: 'left', fontWeight: '600', width: '30%' }}>Environment Name</th>
              <th style={{ padding: '12px', textAlign: 'left', fontWeight: '600', width: '38%' }}>Purpose</th>
                    ...
                    <div style={{ paddingTop: '6px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <TextField
                          size="small"
                          style={{ flex: 1 }}
                          value={editValues.environmentCode || ''}
                          onChange={(e) => {
                            const value = e.target.value.toUpperCase().replace(/[^a-zA-Z0-9\-]/g, '');
                            if (value.length <= 30) {
                              handleInputChange('environmentCode', value);
                            }
                          }}
                        ...
                        />
                        <div style={{ fontSize: '12px', color: (editValues.environmentCode || '').length >= 30 ? '#ef4444' : '#6b7280', whiteSpace: 'nowrap' }}>
                          {(editValues.environmentCode || '').length}/30
                        </div>
                      </div>
                      {(editValues.environmentCode || '').length >= 30 && (
                      ...
                      {validationErrors[editingItem]?.environmentCode && (
                        <div style={{ fontSize: '12px', color: '#ef4444', whiteSpace: 'nowrap', marginTop: '4px' }}>
                    ...
                    <div style={{ paddingTop: '6px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <TextField
                          size="small"
                          style={{ flex: 1 }}
                          value={editValues.environmentName || ''}
                          onChange={(e) => {
                            const value = e.target.value;
                            if (value.length <= 100) {
                              handleInputChange('environmentName', capitalizeFirstChar(value));
                            }
                          }}
                        ...
                        />
                        <div style={{ fontSize: '12px', color: (editValues.environmentName || '').length >= 100 ? '#ef4444' : '#6b7280', whiteSpace: 'nowrap' }}>
                          {(editValues.environmentName || '').length}/100
                        </div>
                      </div>
                      {(editValues.environmentName || '').length >= 100 && (
                        <div style={{ fontSize: '12px', color: '#ef4444', whiteSpace: 'nowrap', marginTop: '4px' }}>
                      ...
                      {validationErrors[editingItem]?.environmentName && (
                        <div style={{ fontSize: '12px', color: '#ef4444', whiteSpace: 'nowrap', marginTop: '4px' }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <TextField
                          ...
                          onChange={(e) => {
                            const value = e.target.value;
                            if (value.length <= 240) {
                              handleInputChange('purpose', capitalizeFirstChar(value));
                            }
                          }}
                          placeholder="Purpose"
                          variant="outlined"
                        ...
                        />
                        <div style={{ fontSize: '12px', color: (editValues.purpose || '').length >= 240 ? '#ef4444' : '#6b7280', whiteSpace: 'nowrap' }}>
                          {(editValues.purpose || '').length}/240
                        </div>
                      </div>
                      {(editValues.purpose || '').length >= 240 && (
                        <div style={{ fontSize: '12px', color: '#ef4444', whiteSpace: 'nowrap', marginTop: '4px' }}>
            ...
            {data.length === 0 && !loading && (
              <tr>
                <td colSpan="4" style={{ padding: '40px', textAlign: 'center', color: '#666' }}>
                  No environments found. Click "Add Environment" to create one.
                </td>
              </tr>
            )}
          </tbody>
      ...
      <div className="table-actions-bottom" style={{ display: 'flex', gap: '12px', marginTop: '16px', marginBottom: '24px', alignItems: 'center', marginLeft: '2rem' }}>
        ...
        >
          ...
          <span>{hasNewRow ? 'Save Environment' : 'Add Environment'}</span>
        </button>
      </div>
