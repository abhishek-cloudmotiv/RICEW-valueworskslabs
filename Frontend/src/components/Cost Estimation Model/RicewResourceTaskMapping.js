...
const RicewResourceTaskMapping = ({ onClose, selectedProject, setUnsavedChangesChecker }) => {
  ...
  const fetchLovData = async () => {
    ...
    try {
      ...
      try {
        ...
        console.error('Failed to get ID token for fetchLovData:', tokenError);
      }
      ...
      const response = await fetch(`https://wnzuudcblf.execute-api.ap-south-1.amazonaws.com/New/ricew/levelDefinition/getSpecific`, {
        method: 'GET',
        headers: headers
      });
      ...
      if (response.ok) {
        ...
        console.log('Fetched LOV data:', result);

        if (result.success && Array.isArray(result.data)) {
          // Sort by Level_Definition_id ascending (small values first)
          const sortedData = result.data.sort((a, b) => {
            const aId = parseInt(a.Level_Definition_id) || 0;
            const bId = parseInt(b.Level_Definition_id) || 0;
            return aId - bId;
          });

          // Transform to the format expected by Select components
          const transformedLov = sortedData.map(item => ({
            value: item.Level_Short_Code,
            label: item.Level_Short_Code
          }));
          setLovData(transformedLov);
        } else {
          setLovError('Invalid LOV data format');
        }
      } else {
        setLovError('Failed to load LOV data');
      }
    } catch (error) {
      console.error('Error fetching LOV data:', error);
      setLovError('Error loading LOV data');
    } finally {
      setLovLoading(false);
    }
  };
  ...
  const checkUnsavedChanges = (callback) => {
    if (editingItem !== null) {
      ...
      // Auto-save or discard logic could go here, but for now just callback
      callback();
    } else {
      callback();
    }
  ...
  const handleCopyFromMaster = async () => {
    try {
      ...
      // Check if endpoint is same as estimation model or different
      // Assuming a similar endpoint exists for resource task mapping
      const response = await fetch('https://wnzuudcblf.execute-api.ap-south-1.amazonaws.com/New/ricew/resourceTaskMapping/copyFromMaster', {
        method: 'POST',
        headers: headers,
  ...
  const loadData = async (skipCopy = false) => {
    // Should NOT happen given the useEffect check, but as a safety guard:
    if (!isSelectionComplete) {
      console.log('Selection incomplete, skipping loadData');
      return;
    }
    ...
    try {
      ...
      const response = await fetch(`https://wnzuudcblf.execute-api.ap-south-1.amazonaws.com/New/newApi/ricew/resourceTaskMapping/getByProject?project_id=${currentProjectId}&organization_id=${encodeURIComponent(selectedOrganization)}&Service_Line_name=${encodeURIComponent(selectedServiceLine)}`, {
        method: 'GET',
        headers: headers
      });
      ...
      if (response.ok) {
        const result = await response.json();
        console.log('Fetched RICEW Resource Task Mapping data:', result);

        const apiData = result.success && Array.isArray(result.data) ? result.data : [];

        if (apiData.length === 0 && !skipCopy && isSelectionComplete) {
        ...
        const transformedData = apiData.map((item, index) => ({
          ...
          master_resource_task_id: item.Master_RICEW_Resource_Task_id || null,
        ...
        const hasDraftRecords = sortedData.some(item => item.saveDraft === "true" || item.saveDraft === true);
        setIsDraftOrganization(hasDraftRecords);

        const isTableLocked = sortedData.some(item => item.isLocked === "true" || item.isLocked === true);
        setIsLocked(isTableLocked);

        setData(sortedData);
        setOriginalData(sortedData.map(item => ({ ...item })));
        setIsMasterData(false);
        console.log('RICEW Resource Task Mapping data loaded successfully');
      } else {
        console.error('Failed to fetch data');
        setErrorMessage('Failed to load Resource Task Mapping data');
        setShowErrorMessage(true);
        setTimeout(() => {
      ...
      console.error('Error fetching RICEW Resource Task Mapping data:', error);
      setErrorMessage('Error loading Resource Task Mapping data');
      setShowErrorMessage(true);
      setTimeout(() => {
        setShowErrorMessage(false);
        setErrorMessage('');
  ...
  // Fetch LOV data when project changes
  useEffect(() => {
    if (!selectedProject?.id && !projectId) {
      return;
    }
  ...
  }, [selectedProject?.id, projectId, selectedOrganization, selectedServiceLine]);
  ...
  const handleSaveDraft = () => {
    checkUnsavedChanges(async () => {
      try {
        ...
        const response = await fetch('https://wnzuudcblf.execute-api.ap-south-1.amazonaws.com/New/newApi/ricew/resourceTaskMapping/updateSaveDraft/byProject', {
          method: 'POST',
          headers: headers,
          body: JSON.stringify(payload)
        });
        ...
        if (response.ok) {
          ...
          setSuccessMessage(`${result.updatedCount || 0} resource task mappings set to draft successfully!`);
  ...
  const handleLockUnlock = () => {
    checkUnsavedChanges(async () => {
      try {
        ...
        const response = await fetch('https://wnzuudcblf.execute-api.ap-south-1.amazonaws.com/New/newApi/ricew/resourceTaskMapping/updateLocked/byProject', {
          method: 'POST',
          headers: headers,
          body: JSON.stringify(payload)
        });
  ...
  const saveToLocalStorage = (updatedData) => {
    const storageKey = `ricewResourceTaskMappingData_${currentProjectId}`;
  ...
  const handleEdit = (id, fieldName = null) => {
    // If attempting to edit a different item while changes are pending on current item
    if (editingItem !== null && editingItem !== id) {
      // Just save the current edit before moving to new one
      handleSaveEdit(editingItem);
      // Then start editing the new one (need to wait for state update in theory, but for now we can just proceed)
      // But user wants click-to-edit behavior which implies auto-save or quick switch
    }

    checkUnsavedChanges(() => {
      // Ensure we don't edit if table is locked or in master view mode
      if (isLocked || isMasterData) return;

      const item = data.find(d => d.id === id);
      if (item) {
        if (!item.isActive) {
          // Don't allow editing inactive items
          setErrorMessage('Cannot edit inactive records.');
          setShowErrorMessage(true);
          setTimeout(() => {
            setShowErrorMessage(false);
  ...
  const handleSaveEdit = (id) => {
    // Check if we're not showing master data
    if (isMasterData) {
  ...
  const saveActiveChanges = () => {
    checkUnsavedChanges(async () => {
      ...
      try {
        ...
        // Prepare payload for changed items
        const payloads = Array.from(changedItems).map(id => {
          const item = data.find(d => d.id === id);
          return {
            RICEW_Resource_Task_id: item.resource_task_id,
            updated_by: userId,
        ...
        }).filter(p => p.RICEW_Resource_Task_id);

        if (payloads.length === 0 && changedItems.size > 0) {
          // Should not happen if data is consistent
          console.warn("Changed items found but no valid IDs to update");
        }

        if (payloads.length > 0) {
          const response = await fetch('https://wnzuudcblf.execute-api.ap-south-1.amazonaws.com/New/newApi/ricew/resourceTaskMapping/multiUpdate', {
            method: 'POST',
            headers: headers,
    ...
    checkUnsavedChanges(async () => {
      try {
        ...
        if (response.ok) {
          ...
          setSuccessMessage(`${result.updatedCount || 0} resource task mappings finalized successfully!`);
          setShowSuccessMessage(true);
          setTimeout(() => {
            setShowSuccessMessage(false);
            setSuccessMessage('');
  ...
  return (
    ...
    <div className="config-main" style={{ minHeight: '80vh' }}>
      ...
      <div className="config-header" style={{ marginTop: '0', display: 'flex', alignItems: 'center', gap: '100px', justifyContent: 'flex-start' }}>
        <h2 style={{ margin: 0 }}>RICEW Resource Level / Task Mapping</h2>
                  ...
                  <div style={{ color: '#444', fontSize: '14px', lineHeight: '1.6' }}>
                    ...
                    <div style={{ marginBottom: '16px' }}>
                      ...
                      <p style={{ margin: '6px 0 0 0', color: '#555' }}>
                        The <strong>RICEW Resource Level / Task Mapping</strong> page allows you to define and manage the relationship between RICEW items, resource levels, and task types. It enables you to specify which resource levels are assigned to specific tasks within each RICEW component, supporting detailed resource allocation and task assignment planning across your project scope.
                      </p>
                    </div>
                    ...
                    <div style={{ marginBottom: '16px' }}>
                      ...
                      <p style={{ margin: '6px 0 0 0', color: '#555' }}>
                        Resource-task mapping is essential for project execution and resource planning. It enables you to: (1) assign appropriate resource levels to specific tasks and RICEW components, (2) optimize resource utilization by clearly defining task ownership, (3) create accurate resource forecasts and capacity plans, (4) support team allocation and scheduling decisions, and (5) provide visibility into resource requirements across all project components.
                      </p>
                    </div>
                    ...
                    <div style={{ marginBottom: '16px' }}>
                      ...
                      <ul style={{ margin: '6px 0 0 0', paddingLeft: '18px', color: '#555' }}>
                        <li><strong>RICEW Item</strong> — The specific RICEW component (Requirements, Implementation, Enhancement, or Warranty/Support) being mapped to resources.</li>
                        <li><strong>Complexity</strong> — The difficulty level of the RICEW item (Low, Medium, High) which influences resource level assignment.</li>
                        <li><strong>Resource Level</strong> — The seniority or skill level of the resource being assigned (e.g., L1, L2, L3 corresponding to Junior, Mid-level, Senior).</li>
                        <li><strong>Task Type</strong> — The type of work task the resource will perform (e.g., Development, Testing, Documentation, Code Review).</li>
                        <li><strong>Count</strong> — The number of resources at the specified level assigned to this task type for the RICEW item.</li>
                        <li><strong>Active</strong> — Status indicator showing whether this resource-task mapping is active or inactive.</li>
                      </ul>
                    </div>
                    ...
                    <div style={{ marginBottom: '16px' }}>
                      ...
                      <ul style={{ margin: '6px 0 0 0', paddingLeft: '18px', color: '#555' }}>
                        <li>Select an <strong>Organization Name</strong> from the dropdown to determine which organization's resource mappings you want to configure.</li>
                        <li>Select a <strong>Service Line Name</strong>. The system will load resource-task mappings configured for the selected organization and service line.</li>
                        <li>The table displays all configured RICEW items and their resource-task mappings. Click the <strong>Edit</strong> (pencil) icon on any row to modify the mapping.</li>
                        <li>When editing a row: Specify the <strong>Resource Level</strong> to assign and the <strong>Task Type</strong> they will perform, along with the <strong>Count</strong> of such resources.</li>
                    ...
                    <div style={{ marginBottom: '4px' }}>
                      ...
                      <ul style={{ margin: '6px 0 0 0', paddingLeft: '18px', color: '#555' }}>
                        <li>Both <strong>Organization Name</strong> and <strong>Service Line Name</strong> are mandatory. The system will not load resource mappings until both are selected.</li>
                        <li>Resource levels are defined in your organization's Resource Level Definition and must be valid assignments for the selected service line.</li>
                        <li>Task types are predefined based on organization configuration and represent the types of work performed in your projects.</li>
                        ...
                        <li>Resource count represents the planned number of resources at each level for that specific task and RICEW component combination.</li>
                        <li>Ensure all critical RICEW items have appropriate resource-task mappings defined before submitting the configuration.</li>
                      </ul>
                    </div>
      ...
      {/* Loading Overlay */}
      <Loader 
        loading={loading || lockingUnlocking || savingDraft || savingActiveChanges} 
        message={lockingUnlocking ? (isLocked ? 'Unlocking...' : 'Locking...') : 
                 (savingDraft || savingActiveChanges) ? 'Processing...' : 'Loading Data...'} 
      />
                ...
                <tr key={item.id} data-row-id={item.id} style={{
                  ...
                  {/* <td style={{
                    ...
                    <div className="action-icons" style={{ display: 'flex', gap: '8px', alignItems: 'center', justifyContent: 'center' }}>
                      ...
                      ) : item.resource_task_id && item.isActive && (
                        <button
                          ...
                          title={isMasterData ? "Actions disabled while viewing master data" : isLocked ? "Actions disabled while table is locked" : "Edit"}
                        >
      ...
      <div className="table-actions-bottom" style={{ display: 'flex', gap: '12px', marginTop: '16px', alignItems: 'center', marginLeft: '16px' }}>
        ...
        <button
          ...
          onMouseEnter={(e) => { if (changedItems.size > 0 && !savingActiveChanges) e.target.style.backgroundColor = '#218838'; }}
          onMouseLeave={(e) => { if (changedItems.size > 0 && !savingActiveChanges) e.target.style.backgroundColor = '#28a745'; }}
        >
...
export default RicewResourceTaskMapping;