...
import { DeveloperResourceAutocomplete } from '../Developer Specification/DeveloperSpecificationLOVlist';
import { HelpCircle, X } from 'lucide-react';
import { getIdToken } from '../../../utils/cognito-auth';
import DOMPurify from 'dompurify';
import { useSession } from '../../../context/SessionContext';

const FunctionalTestingSpecificationAssignmentForm = ({ selectedProject, onBackToLanding }) => {
    ...
    const fetchRicewData = useCallback(async () => {
        ...
        try {
            ...
            const [ricewResponse, assignmentResponse] = await Promise.all([
                fetch(`https://07eirxky2f.execute-api.ap-south-1.amazonaws.com/New/api/ricew/requestForm/getCodeCompleteByProject?project_id=${projectId}`, { headers }),
                fetch(`https://h78086ug8d.execute-api.ap-south-1.amazonaws.com/New/api/ricew/functionalTestingAssignment/getRecord-byProject?Project_id=${projectId}`, { headers })
            ]);
            ...
            if (ricewResult.success) {
                ...
                const mergedData = ricewList.map(item => {
                    ...
                    return {
                        ...
                        developerResourceId: assignment ? (assignment.Resource_Roster_Form_id || assignment.Choose_Resource_Functional_Testing) : (item.Developer_Resource_Id || ''),
                        developerResource: DOMPurify.sanitize(assignment ? assignment.Choose_Resource_Functional_Testing : (item.Developer_Resource || ''), { ALLOWED_TAGS: [] }),
                        developerResourceName: DOMPurify.sanitize(assignment ? assignment.Choose_Resource_Functional_Testing : (item.Developer_Resource || ''), { ALLOWED_TAGS: [] }),
                        ...
                        assignmentTimestamp: DOMPurify.sanitize(assignment ? (assignment.assign_object_date || assignment.created_timestamp || '') : '', { ALLOWED_TAGS: [] }),
                        assignmentId: assignment ? (assignment.Functional_Testing_Assignment_id || '') : '',
                        ...
                        developerUserId: assignment ? (assignment.Functional_Testing_user_id || '') : ''
                    };
        ...
        try {
            ...
            const payload = {
                records: [{
                    ...
                    Choose_Resource_Functional_Testing: DOMPurify.sanitize(edits.displayName || '', { ALLOWED_TAGS: [] }),
                    ...
                    Functional_Testing_user_id: edits.userId || '',
                    Functional_Testing_name: DOMPurify.sanitize(edits.displayName || '', { ALLOWED_TAGS: [] })
                }]
            };

            const response = await fetch('https://h78086ug8d.execute-api.ap-south-1.amazonaws.com/New/api/ricew/functionalTestingAssignment/createSubmit', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${idToken}`
        ...
        setConfirmAction(() => async () => {
            ...
            try {
                ...
                const recordsToAssign = selectedRows.map(rowId => {
                    ...
                    return {
                        RICEW_Object: DOMPurify.sanitize(row.RICEW_Object || '', { ALLOWED_TAGS: [] }),
                        Choose_Resource_Functional_Testing: DOMPurify.sanitize(row.developerResourceName || '', { ALLOWED_TAGS: [] }),
                        ...
                        Functional_Testing_user_id: row.developerUserId || '',
                        Functional_Testing_name: DOMPurify.sanitize(row.developerResourceName || '', { ALLOWED_TAGS: [] })
                    };
                }).filter(Boolean);

                const response = await fetch('https://h78086ug8d.execute-api.ap-south-1.amazonaws.com/New/api/ricew/functionalTestingAssignment/assign', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': `Bearer ${idToken}`
                ...
                if (response.ok && result.success) {
                    ...
                    const currentProjectName = DOMPurify.sanitize(localStorage.getItem('project_name') || selectedProject?.name || '', { ALLOWED_TAGS: [] });

                    const emailPayload = recordsToAssign.map(record => ({
                        toEmail: DOMPurify.sanitize(record.Email_Address || '', { ALLOWED_TAGS: [] }),
                        Functional_Testing_name: DOMPurify.sanitize(record.Functional_Testing_name || '', { ALLOWED_TAGS: [] }),
                        projectName: currentProjectName,
                        ricewObject: DOMPurify.sanitize(record.RICEW_Object || '', { ALLOWED_TAGS: [] })
                    }));

                    try {
                        const emailResponse = await fetch('https://h78086ug8d.execute-api.ap-south-1.amazonaws.com/New/api/send/functional-testing-assignment', {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                                'Authorization': `Bearer ${idToken}`
            ...
            <div style={{ padding: '1rem 1rem 2rem 1rem', width: '100%', minWidth: '1320px', margin: '0', boxSizing: 'border-box' }}>
                ...
                <div style={{
                    ...
                    <div className="config-header" style={{ marginTop: '0', marginRight: "0px", display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.5rem 2rem' }}>
                        <h2 style={{ margin: 0 }}>Functional Testing Assignment Form</h2>
                                        ...
                                        <div className="help-modal-scroll" style={{ overflowY: 'auto', padding: '32px', textAlign: 'left', flex: '1' }}>
                                            ...
                                            <div style={{ color: '#444', fontSize: '14px', lineHeight: '1.6' }}>
                                                <div style={{ marginBottom: '16px' }}>
                                                    ...
                                                    <p style={{ margin: '6px 0 0 0', color: '#555' }}>The <strong>Functional Testing Assignment Form</strong> allows delivery managers to assign RICEW objects to testers for functional testing.</p>
                                                </div>
                                                <div style={{ marginBottom: '16px' }}>
...
export default FunctionalTestingSpecificationAssignmentForm;
