...
import { TechnicalResourceAutocomplete } from './TechnicalSpecificationLOVlist';
import { HelpCircle, X } from 'lucide-react';
import { getIdToken } from '../../../utils/cognito-auth';
import DOMPurify from 'dompurify';
import { useSession } from '../../../context/SessionContext';

const TechnicalSpecificationAssignmentForm = ({ selectedProject, onBackToLanding }) => {
    ...
    useEffect(() => {
        ...
        if (editingResourceRowId || showHelpPopup) {
    ...
    const fetchRicewData = useCallback(async () => {
        ...
        try {
            ...
            const [ricewResponse, assignmentResponse] = await Promise.all([
                fetch(`https://x62hhl9a23.execute-api.ap-south-1.amazonaws.com/New/api/ricew/getFSCompleteByProject?project_id=${projectId}`, { headers }),
                fetch(`https://x62hhl9a23.execute-api.ap-south-1.amazonaws.com/New/newApi/ricew/technicalSpecificationAssignment/getRecord/byProject?Project_id=${projectId}`, { headers })
            ]);
            ...
            if (ricewResult.success) {
                ...
                const mergedData = ricewList.map(item => {
                    // New API returns RICEW_Name as the object name
                    const ricewObject = DOMPurify.sanitize(item.RICEW_Name || item.RICEW_Object || '', { ALLOWED_TAGS: [] });
                    const assignment = assignmentList.find(a => a.RICEW_Object === ricewObject);
                    ...
                    return {
                        ...item,
                        RICEW_Object: ricewObject,
                        technicalResourceId: DOMPurify.sanitize(assignment ? (assignment.Resource_Roster_Form_id || assignment.Choose_Resource_Technical) : (item.Technical_Resource_Id || ''), { ALLOWED_TAGS: [] }),
                        technicalResource: DOMPurify.sanitize(assignment ? assignment.Choose_Resource_Technical : (item.Technical_Resource || ''), { ALLOWED_TAGS: [] }),
                        technicalResourceName: DOMPurify.sanitize(assignment ? assignment.Choose_Resource_Technical : (item.Technical_Resource || ''), { ALLOWED_TAGS: [] }),
                        emailDisplay: DOMPurify.sanitize(assignment ? assignment.Email_Address : (item.Technical_Resource_Email || ''), { ALLOWED_TAGS: [] }),
                        ...
                        assignmentId: assignment ? (assignment.TechnicalSpecificationAssignment_id || '') : '',
                        ...
                        technicalUserId: DOMPurify.sanitize(assignment ? (assignment.Technical_user_id || '') : '', { ALLOWED_TAGS: [] })
                    };
                ...
                mergedData.sort((a, b) => {
                    ...
                    if (a.isSubmitted) {
                        ...
                        const timeA = parseDate(a.created_date);
                        const timeB = parseDate(b.created_date);
                        if (timeB !== timeA) return timeB - timeA;
    ...
    useEffect(() => {
        const selectableRows = ricewData.filter(row => !row.isFullyAssigned && row.technicalResource && row.emailDisplay);
    ...
    const handleResourceChange = (id, resourceId, email, displayName, userId) => {
        setEditingValues(prev => ({
            ...
            [id]: {
                resourceId,
                email,
                displayName: displayName || resourceId,
                userId: userId || ''
            }
        }));
    };
    ...
    const handleUnsavedResourceChange = (id, resourceId, email, displayName, userId) => {
        setRicewData(prev => prev.map(row =>
            row.RICEWRequestFormId === id
                ? {
                    ...row,
                    technicalResourceId: resourceId,
                    technicalResource: resourceId,
                    technicalResourceName: displayName || resourceId,
                    emailDisplay: email,
        ...
        try {
            ...
            const payload = {
                records: [{
                    ...
                    Status_Submitted: row.isSubmitted, // Keep existing submission status
            ...
            const response = await fetch('https://x62hhl9a23.execute-api.ap-south-1.amazonaws.com/New/newApi/ricew/technicalSpecificationAssignment/createSubmit', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${idToken}`
    ...
    const handleSelectAll = (e) => {
        ...
        if (isChecked) {
            ...
            const selectableIds = ricewData
                .filter(row => !row.isFullyAssigned && row.technicalResource && row.emailDisplay)
        ...
        setConfirmAction(() => async () => {
            ...
            try {
                ...
                const recordsToAssign = selectedRows.map(rowId => {
                    ...
                    return {
                        RICEW_Object: DOMPurify.sanitize(row.RICEW_Object || '', { ALLOWED_TAGS: [] }),
                        Choose_Resource_Technical: DOMPurify.sanitize(row.technicalResourceName || '', { ALLOWED_TAGS: [] }),
                        Email_Address: DOMPurify.sanitize(row.emailDisplay || '', { ALLOWED_TAGS: [] }),
                        Project_id: projectId,
                        created_by: userId,
                        updated_by: userId,
                        user_id: userId,
                        Resource_Roster_Form_id: DOMPurify.sanitize(row.technicalResourceId || '', { ALLOWED_TAGS: [] }),
                        RICEWRequestFormId: row.RICEWRequestFormId || '',
                        Technical_user_id: DOMPurify.sanitize(row.technicalUserId || '', { ALLOWED_TAGS: [] }),
                        Technical_name: DOMPurify.sanitize(row.technicalResourceName || '', { ALLOWED_TAGS: [] })
                    };
                }).filter(Boolean);

                const response = await fetch('https://x62hhl9a23.execute-api.ap-south-1.amazonaws.com/New/newApi/ricew/technicalSpecificationAssignment/assign', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': `Bearer ${idToken}`
                ...
                if (response.ok && result.success) {
                    ...
                    const emailPayload = recordsToAssign.map(record => ({
                        ...
                        Technical_name: DOMPurify.sanitize(record.Technical_name || '', { ALLOWED_TAGS: [] }),
                        projectName: DOMPurify.sanitize(currentProjectName || '', { ALLOWED_TAGS: [] }),
                        ricewObject: DOMPurify.sanitize(record.RICEW_Object || '', { ALLOWED_TAGS: [] })
                    }));

                    try {
                        await fetch('https://x62hhl9a23.execute-api.ap-south-1.amazonaws.com/New/newApi/send/technical-writer-assignment', {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                                'Authorization': `Bearer ${idToken}`
            ...
            <div style={{ padding: '1rem 1rem 2rem 1rem', width: '100%', minWidth: '1320px', margin: '0', boxSizing: 'border-box' }}>
                ...
                <div style={{
                    ...
                    <div className="config-header" style={{
                        ...
                        <h2 style={{ margin: 0 }}>Technical Specification Assignment Form</h2>
                                            ...
                                            <div style={{ color: '#444', fontSize: '14px', lineHeight: '1.6' }}>
                                                <div style={{ marginBottom: '16px' }}>
                                                    ...
                                                    <p style={{ margin: '6px 0 0 0', color: '#555' }}>
                                                        The <strong>Technical Specification Assignment Form</strong> allows you to assign technical resources to RICEW objects.
                                                    </p>
                                                </div>
                                                ...
                                                <div style={{ marginBottom: '16px' }}>
                                                    ...
                                                    <ul style={{ margin: '6px 0 0 0', paddingLeft: '18px', color: '#555' }}>
                                                        <li><strong>RICEW Object</strong> — Clickable link to view the details for the RICEW object.</li>
                                                        <li><strong>Choose Resource</strong> — Dropdown to select a technical resource from the project roster.</li>
                                                        <li><strong>Select</strong> — Checkbox to select multiple records for bulk assignment.</li>
                                                    </ul>
                                                </div>
                                                ...
                                                <div style={{ marginBottom: '16px' }}>
                                                    ...
                                                    <ul style={{ margin: '6px 0 0 0', paddingLeft: '18px', color: '#555' }}>
                                                        <li>Select a resource from the dropdown for the desired rows.</li>
                                                        <li>Check the box in the <strong>Select</strong> column for those rows.</li>
                                                        <li>Click the <strong>Assign Object</strong> button at the top to complete the assignment and notify the resources.</li>
                                                    </ul>
                                                </div>
                                            </div>
                                        </div>
                    ...
                    <div style={{ padding: '20px' }}>
                        ...
                        <div style={{
                            ...
                            <div style={{
                                ...
                                <div style={{ flex: 1, padding: '12px 12px', fontWeight: 'bold', fontSize: '14px', color: '#333', borderRight: '1px solid #ddd', minWidth: '320px', backgroundColor: 'white' }}>Choose Resource (Technical) From the Roster</div>
                                ...
                                <div style={{
                                    ...
                                    fontSize: '14px',
                                    color: '#333',
                                    backgroundColor: !ricewData.some(row => !row.isFullyAssigned && row.technicalResource && row.emailDisplay) ? '#f0f0f0' : 'white',
                                    display: 'flex',
                                    flexDirection: 'column',
                                    alignItems: 'center',
                                    justifyContent: 'center',
                                    ...
                                    <input
                                        ...
                                        disabled={!ricewData.some(row => !row.isFullyAssigned && row.technicalResource && row.emailDisplay)}
                                        style={{
                                            cursor: !ricewData.some(row => !row.isFullyAssigned && row.technicalResource && row.emailDisplay) ? 'not-allowed' : 'pointer',
                                            width: '16px',
                                            height: '16px',
                                            marginTop: '6px'
                                        }}
                                        ...
                                        >
                                            ...
                                            <div style={{ flex: 1, padding: '12px 12px', fontSize: '13px', borderRight: '1px solid #ddd', minWidth: '250px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', display: 'flex', alignItems: 'center' }}>
                                                <span
                                                    ...
                                                    onClick={() => navigate(`/dashboard/functional-specification-view/${row.RICEWRequestFormId}?from=technical-assignment`)}
                                                ...
                                                {editingResourceRowId === row.RICEWRequestFormId ? (
                                                    <div style={{
                                                        ...
                                                        <div style={{ flex: 1, minWidth: 0, overflow: 'hidden' }}>
                                                            <TechnicalResourceAutocomplete
                                                                value={editingValues[row.RICEWRequestFormId]?.resourceId ?? row.technicalResourceId}
                                            ...
                                            <div style={{
                                                ...
                                                alignItems: 'center',
                                                justifyContent: 'center',
                                                backgroundColor: (!row.technicalResource || !row.emailDisplay) && !row.isFullyAssigned ? '#f0f0f0' : 'transparent'
                                            }}>
                                                {row.isFullyAssigned || row.submittedStatusText === 'Assigned' ? (
                                                    ...
                                                    <input
                                                        ...
                                                        disabled={!row.technicalResource || !row.emailDisplay}
                                                        style={{
                                                            cursor: (!row.technicalResource || !row.emailDisplay) ? 'not-allowed' : 'pointer',
                                                            width: '16px',
                                                            height: '16px'
                                                        }}
                                                    />
...
export default TechnicalSpecificationAssignmentForm;
