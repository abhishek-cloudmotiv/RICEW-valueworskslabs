...
import { FunctionalResourceAutocomplete } from './FunctionalSpecificationLOVlist';
import { HelpCircle, X } from 'lucide-react';
import { getIdToken } from '../../../utils/cognito-auth';
import DOMPurify from 'dompurify';
import { useSession } from '../../../context/SessionContext';

const FunctionalSpecificationAssignmentForm = ({ selectedProject, onBackToLanding }) => {
    ...
    const fetchRicewData = useCallback(async () => {
        ...
        try {
            ...
            const [ricewResponse, assignmentResponse] = await Promise.all([
                fetch(`https://oi5gtp4f3l.execute-api.ap-south-1.amazonaws.com/New/newApi/ricew/requestForm/byProject?project_id=${projectId}`, { headers }),
                fetch(`https://oi5gtp4f3l.execute-api.ap-south-1.amazonaws.com/New/newApi/ricew/functionalSpecificationAssignment/getRecord/byProject?Project_id=${projectId}`, { headers })
            ]);
            ...
            if (ricewResult.success) {
                ...
                const mergedData = ricewList.map(item => {
                    const assignment = assignmentList.find(a => a.RICEW_Object === item.RICEW_Name);
                    ...
                    return {
                        ...item,
                        functionalResourceId: DOMPurify.sanitize(assignment ? (assignment.Resource_Roster_Form_id || assignment.Choose_Resource_Functional) : (item.Functional_Resource_Id || ''), { ALLOWED_TAGS: [] }),
                        functionalResource: DOMPurify.sanitize(assignment ? assignment.Choose_Resource_Functional : (item.Functional_Resource || ''), { ALLOWED_TAGS: [] }),
                        functionalResourceName: DOMPurify.sanitize(assignment ? assignment.Choose_Resource_Functional : (item.Functional_Resource || ''), { ALLOWED_TAGS: [] }),
                        emailDisplay: DOMPurify.sanitize(assignment ? assignment.Email_Address : (item.Functional_Resource_Email || ''), { ALLOWED_TAGS: [] }),
                        assignmentTimestamp: assignment ? (assignment.created_timestamp || '') : '',
                        assignmentId: assignment ? (assignment.FunctionalSpecificationAssignment_id || '') : '',
                        isSubmitted: !!assignment,
                        isFullyAssigned: rawStatus === 'Assign' || isWorkAssigned,
                        isWorkAssigned: isWorkAssigned,
                        submittedStatusText: (rawStatus === 'Assign' || isWorkAssigned) ? 'Assigned' : (rawStatus === 'true' || rawStatus === true ? 'Processed' : null),
                        functionalUserId: DOMPurify.sanitize(assignment ? (assignment.Functional_user_id || '') : '', { ALLOWED_TAGS: [] })
                    };
    ...
    useEffect(() => {
        const selectableRows = ricewData.filter(row => !row.isFullyAssigned && row.functionalResource && row.emailDisplay);
    ...
    const handleUnsavedResourceChange = (id, resourceId, email, displayName, userId) => {
        setRicewData(prev => prev.map(row =>
            row.RICEWRequestFormId === id
                ? {
                    ...row,
                    functionalResourceId: resourceId,
                    functionalResource: resourceId,
                    functionalResourceName: displayName || resourceId,
                    emailDisplay: email,
                    functionalUserId: userId || ''
                }
                : row
        ));
    };
    ...
    const handleSaveEdit = async (rowId) => {
        ...
        try {
            ...
            const payload = {
                records: [{
                    RICEW_Object: DOMPurify.sanitize(row.RICEW_Name || '', { ALLOWED_TAGS: [] }),
                    Choose_Resource_Functional: DOMPurify.sanitize(edits.displayName || '', { ALLOWED_TAGS: [] }),
                    ...
                    Functional_user_id: DOMPurify.sanitize(edits.userId || '', { ALLOWED_TAGS: [] }),
                    Functional_name: DOMPurify.sanitize(edits.displayName || '', { ALLOWED_TAGS: [] })
                }]
            };

            const response = await fetch('https://oi5gtp4f3l.execute-api.ap-south-1.amazonaws.com/New/newApi/ricew/functionalSpecificationAssignment/createSubmit', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${idToken}`
    ...
    const handleSelectAll = (e) => {
        ...
        if (isChecked) {
            ...
            const selectableIds = ricewData
                .filter(row => !row.isFullyAssigned && row.functionalResource && row.emailDisplay)
        ...
        setConfirmAction(() => async () => {
            ...
            try {
                ...
                const recordsToAssign = selectedRows.map(rowId => {
                    ...
                    return {
                        RICEW_Object: DOMPurify.sanitize(row.RICEW_Name || '', { ALLOWED_TAGS: [] }),
                        Choose_Resource_Functional: DOMPurify.sanitize(row.functionalResourceName || '', { ALLOWED_TAGS: [] }),
                        Email_Address: DOMPurify.sanitize(row.emailDisplay || '', { ALLOWED_TAGS: [] }),
                        Project_id: projectId,
                        created_by: userId,
                        updated_by: userId,
                        user_id: userId,
                        Resource_Roster_Form_id: DOMPurify.sanitize(row.functionalResourceId || '', { ALLOWED_TAGS: [] }),
                        RICEWRequestFormId: row.RICEWRequestFormId || '',
                        Functional_user_id: DOMPurify.sanitize(row.functionalUserId || '', { ALLOWED_TAGS: [] }),
                        Functional_name: DOMPurify.sanitize(row.functionalResourceName || '', { ALLOWED_TAGS: [] })
                    };
                }).filter(Boolean);

                const response = await fetch('https://oi5gtp4f3l.execute-api.ap-south-1.amazonaws.com/New/newApi/ricew/functionalSpecificationAssignment/assign', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': `Bearer ${idToken}`
                ...
                if (response.ok && result.success) {
                    ...
                    const emailPayload = recordsToAssign.map(record => ({
                        ...
                        userName: DOMPurify.sanitize(record.Functional_name, { ALLOWED_TAGS: [] }),
                    ...
                    try {
                        await fetch('https://oi5gtp4f3l.execute-api.ap-south-1.amazonaws.com/New/send/ricew-assignment', {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                                'Authorization': `Bearer ${idToken}`
            ...
            <div style={{ padding: '1rem 1rem 2rem 1rem', width: '100%', minWidth: '1320px', margin: '0', boxSizing: 'border-box' }}>
                ...
                <div style={{
                    ...
                    <div className="config-header" style={{
                        ...
                        <h2 style={{ margin: 0 }}>Functional Specification Assignment Form</h2>
                                            ...
                                            <div style={{ color: '#444', fontSize: '14px', lineHeight: '1.6' }}>
                                                <div style={{ marginBottom: '16px' }}>
                                                    ...
                                                    <p style={{ margin: '6px 0 0 0', color: '#555' }}>
                                                        The <strong>Functional Specification Assignment Form</strong> allows delivery managers to assign RICEW objects to specification writers and manage object assignments.
                                                    </p>
                                                </div>
                                                ...
                                                <div style={{ marginBottom: '16px' }}>
                                                    ...
                                                    <ul style={{ margin: '6px 0 0 0', paddingLeft: '18px', color: '#555' }}>
                                                        <li>Select a <strong>Resource</strong> from the dropdown for each RICEW object.</li>
                                                        <li>Use the checkboxes to select rows, then click <strong>Assign Object</strong> to submit the assignment.</li>
                                                        <li>Assigned objects will be reflected with their assignment date and status.</li>
                                                    </ul>
                                                </div>
                                            </div>
                                        </div>
                    ...
                    <div style={{ padding: '20px' }}>
                        ...
                        <div style={{
                            ...
                            <div style={{
                                ...
                                <div style={{ flex: 1, padding: '12px 12px', fontWeight: 'bold', fontSize: '14px', color: '#333', borderRight: '1px solid #ddd', minWidth: '320px', backgroundColor: 'white' }}>Choose Resource (Functional) From the Roster</div>
                                ...
                                <div style={{
                                    ...
                                    fontSize: '14px',
                                    color: '#333',
                                    backgroundColor: !ricewData.some(row => !row.isFullyAssigned && row.functionalResource && row.emailDisplay) ? '#f0f0f0' : 'white',
                                    display: 'flex',
                                    flexDirection: 'column',
                                    alignItems: 'center',
                                    justifyContent: 'center',
                                    ...
                                    <input
                                        ...
                                        disabled={!ricewData.some(row => !row.isFullyAssigned && row.functionalResource && row.emailDisplay)}
                                        style={{
                                            cursor: !ricewData.some(row => !row.isFullyAssigned && row.functionalResource && row.emailDisplay) ? 'not-allowed' : 'pointer',
                                            width: '16px',
                                            height: '16px',
                                            marginTop: '6px'
                                        }}
                                        ...
                                        >
                                            ...
                                            <div style={{ flex: 1, padding: '12px 12px', fontSize: '13px', borderRight: '1px solid #ddd', minWidth: '250px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', display: 'flex', alignItems: 'center' }}>
                                                <span
                                                    ...
                                                    onClick={() => navigate(`/dashboard/functional-specification-view/${row.RICEWRequestFormId}`)}
                                            ...
                                            >
                                                {editingResourceRowId === row.RICEWRequestFormId ? (
                                                    <div style={{
                                                        ...
                                                        <div style={{ flex: 1, minWidth: 0, overflow: 'hidden' }}>
                                                            <FunctionalResourceAutocomplete
                                                                value={editingValues[row.RICEWRequestFormId]?.resourceId ?? row.functionalResourceId}
                                                    ...
                                                    <FunctionalResourceAutocomplete
                                                        value={row.functionalResourceId || ''}
                                            ...
                                            <div style={{
                                                ...
                                                alignItems: 'center',
                                                justifyContent: 'center',
                                                backgroundColor: (!row.functionalResource || !row.emailDisplay) && !row.isFullyAssigned ? '#f0f0f0' : 'transparent'
                                            }}>
                                                {row.isFullyAssigned || row.submittedStatusText === 'Assigned' ? (
                                                    ...
                                                    <input
                                                        ...
                                                        disabled={!row.functionalResource || !row.emailDisplay}
                                                        style={{
                                                            cursor: (!row.functionalResource || !row.emailDisplay) ? 'not-allowed' : 'pointer',
                                                            width: '16px',
                                                            height: '16px'
                                                        }}
                                                    />
...
export default FunctionalSpecificationAssignmentForm;
