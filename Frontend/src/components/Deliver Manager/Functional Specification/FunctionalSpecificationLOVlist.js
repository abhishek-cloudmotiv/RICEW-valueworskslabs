...
// Resource Autocomplete Component for Functional Specification Assignment
export const FunctionalResourceAutocomplete = ({
    value,
    emailValue = "",
    onChange,
    projectId,
    ...
    useEffect(() => {
        ...
        fetchRosterData();
    }, [projectId, externalOptions, handleAuthError]);
            ...
            requestAnimationFrame(() => {
                requestAnimationFrame(() => {
                    ...
                    console.log('Dropdown check:', {
                        dropdownBottom,
                        viewportHeight,
                        needsScroll: dropdownBottom > viewportHeight
                    });
                    ...
                    if (dropdownBottom > viewportHeight) {
                        ...
                        console.log('Scrolling by:', neededSpace);
    ...
    const handleCloseDropdown = (wasSelectionMade = false) => {
        ...
        if (!wasSelectionMade && previousValueRef.current !== undefined) {
            if (value !== previousValueRef.current || emailValue !== previousEmailRef.current) {
                // Restore the previous value, email, and displayName exactly as they were
                const matchingOption = options.find(opt => opt.value === previousValueRef.current);
                onChangeRef.current(previousValueRef.current, previousEmailRef.current, previousDisplayNameRef.current, matchingOption?.userId || '');
            }
    ...
    const handleInputChange = (e) => {
        ...
        if (matchingOption) {
            onChangeRef.current(matchingOption.value, matchingOption.email, matchingOption.displayName, matchingOption.userId);
    ...
    const handleSelectOption = (option) => {
        ...
        onChangeRef.current(option.value, option.email, option.displayName, option.userId);
