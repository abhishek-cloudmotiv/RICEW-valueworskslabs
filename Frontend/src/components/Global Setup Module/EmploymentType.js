...
const EmploymentType = ({ onClose, selectedProject }) => {
  ...
  const { data: employmentData = [], isLoading, isFetching } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await fetch(GLOBAL_SETUP_API_CONFIG.EMPLOYMENT_TYPES_API_URL, {
        headers: {
      ...
      if (Array.isArray(dataArray) && dataArray.length > 0) {
        ...
        return [
          { id: 1, typeName: 'Employee', description: 'Regular full-time or part-time employees of the company.' },
          { id: 2, typeName: 'Contractor', description: 'External resources hired under a contract for a specific period or project.' },
          { id: 3, typeName: 'Subcon', description: 'Subcontracted resources from third-party vendor agencies.' }
        ];
      }
    }
  });
  ...
  return (
    <div className="config-main" style={{ margin: '2rem 2rem 0 2rem' }}>
      ...
      <div className="config-header" style={{ marginTop: '0', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <h2 style={{ margin: 0, color: '#333' }}>Employment Type</h2>
                  ...
                  <div style={{ color: '#444', fontSize: '14px', lineHeight: '1.6' }}>
                    <div style={{ marginBottom: '14px' }}>
                      ...
                      <p style={{ margin: '6px 0 0 0', color: '#555' }}>
                        The <strong>Employment Type</strong> page defines the classifications of resources engaged on a project, such as direct employees, contractors, or subcontractors.
                      </p>
                    </div>
                    ...
                    <div style={{ marginBottom: '14px' }}>
                      ...
                      <p style={{ margin: '6px 0 0 0', color: '#555' }}>
                        Understanding the mix of employment types is crucial for compliance, costing, resource planning, and identifying organizational dependencies on vendor staff.
                      </p>
                    </div>
                    ...
                    <div style={{ marginBottom: '14px' }}>
                      ...
                      <ul style={{ margin: '6px 0 0 0', paddingLeft: '18px', color: '#555' }}>
                        <li><strong>Employment Type</strong> — The employment classification label (e.g., Employee, Contractor, Subcon).</li>
                        <li><strong>Description</strong> — A detailed explanation of the criteria for each classification.</li>
                      </ul>
                    </div>
                  </div>
                </div>
      ...
      <div className="table-container" style={{ height: 'calc(100vh - 265px)', overflowY: 'auto' }}>
        <table className="config-table" style={{ fontSize: '15px' }}>
          <thead>
            <tr>
              <th style={{ padding: '8px 12px', fontSize: '16px', width: '30%' }}>Employment Type</th>
              <th style={{ padding: '8px 12px', fontSize: '16px', width: '70%' }}>Description</th>
            </tr>
          </thead>
          <tbody>
            {employmentData.length === 0 && !isLoading ? (
              <tr>
              ...
              employmentData.map((item) => (
                <tr key={item.id} style={{ minHeight: '40px' }}>
                  <td style={{ padding: '10px 14px', verticalAlign: 'top', width: '30%', lineHeight: '1.6' }}>
                    {item.typeName}
                  </td>
                  <td style={{ padding: '10px 14px', verticalAlign: 'top', width: '70%', wordWrap: 'break-word', whiteSpace: 'normal', lineHeight: '1.6' }}>
                    {item.description}
                  </td>
                </tr>
              ))
