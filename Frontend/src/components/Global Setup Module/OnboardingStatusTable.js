...
const OnboardingStatusTable = ({ onClose, selectedProject, onBackToLanding, onLogout }) => {
  ...
  const { data: onboardingData = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await fetch(GLOBAL_SETUP_API_CONFIG.ONBOARDING_STATUS_API_URL, {
        headers: {
      ...
      if (!response.ok) {
        throw new Error('Failed to fetch onboarding status data');
      }

      const result = await response.json();
      const dataArray = Array.isArray(result) ? result : (result.data || []);
      
      const filteredDataArray = dataArray.filter(item => item.delete_status !== "true" && item.delete_status !== true);
      const sanitizedData = validateAndSanitizeData(filteredDataArray);

      return sanitizedData.sort((a, b) => {
        const idA = parseInt(a.onboardingStatusId) || 0;
        const idB = parseInt(b.onboardingStatusId) || 0;
        return idA - idB;
      });
    }
  });
  ...
  return (
    <div className="config-main" style={{ margin: '2rem 2rem 0 2rem' }}>
      ...
      <div className="config-header" style={{ marginTop: '0', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <h2 style={{ margin: 0, color: '#333' }}>Onboarding Status Definition</h2>
                  ...
                  <div style={{ color: '#444', fontSize: '14px', lineHeight: '1.6' }}>
                    <div style={{ marginBottom: '14px' }}>
                      ...
                      <p style={{ margin: '6px 0 0 0', color: '#555' }}>
                        The <strong>Onboarding Status Definition</strong> page is used to define the standardized stages of onboarding for project resources.
                      </p>
                    </div>
                    ...
                    <div style={{ marginBottom: '14px' }}>
                      ...
                      <ul style={{ margin: '6px 0 0 0', paddingLeft: '18px', color: '#555' }}>
                        <li><strong>Onboarding Status Type ID</strong> — The unique identifier for the status type.</li>
                        <li><strong>Onboarding Status</strong> — The name of the onboarding stage (e.g., Background Check, Training).</li>
                        <li><strong>Onboarding Status Description</strong> — A detailed explanation of what this stage entails.</li>
                      </ul>
                    </div>
                  </div>
                </div>
      ...
      <div className="table-container" style={{ maxHeight: 'calc(100vh - 280px)', overflowY: 'auto' }}>
        <table className="config-table" style={{ fontSize: '15px', width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr>
              <th style={{ padding: '8px 12px', fontSize: '16px', width: '15%', textAlign: 'left', borderBottom: '1px solid #dee2e6' }}>Onboarding Status Type ID</th>
              <th style={{ padding: '8px 12px', fontSize: '16px', width: '20%', textAlign: 'left', borderBottom: '1px solid #dee2e6' }}>Onboarding Status</th>
              <th style={{ padding: '8px 12px', fontSize: '16px', width: '50%', textAlign: 'left', borderBottom: '1px solid #dee2e6' }}>Onboarding Status Description</th>
            </tr>
          </thead>
          <tbody>
            {onboardingData.length === 0 && !isLoading ? (
              <tr>
                <td colSpan="3" style={{ textAlign: 'center', padding: '20px' }}>
                  No onboarding status definitions found.
                </td>
              </tr>
            ) : (
