...
const NewDashbordResource = ({ onClose, onBackToLanding, onLogout, selectedProject }) => {
  ...
  const displayedData = filteredData.filter(item => {
    ...
    const fullName = (item.IC_full_name || '').toLowerCase();
  ...
  // Fetch LOV data for Row 1 filters
  useEffect(() => {
    const fetchLovData = async () => {
      try {
        ...
        try {
          ...
          console.error('Failed to get ID token for LOV Row 1:', tokenError);
        ...
        const response = await fetch(
          `https://fuahu3jqsc.execute-api.ap-south-1.amazonaws.com/New/api/get/filter/resource-roster-lov-first?project_id=${projectId}`,
          {
            headers: headers
          }
        );
        ...
        if (result.success && result.data) {
          setLovData(prev => ({
            ...prev,
            organizationName: result.data.IC_organization_name || [],
            businessLine: result.data.IC_business_line_name || [],
            portfolioName: result.data.IC_portfolio_name || [],
            serviceLineName: result.data.IC_service_line_name || [],
            employmentType: result.data.IC_employment_type || [],
            workLocation: result.data.IC_work_location || [],
            nationality: result.data.IC_nationality || []
          }));
        }
        setLovLoading(false);
      } catch (error) {
  ...
  // Fetch LOV data for Row 2 filters
  useEffect(() => {
    const fetchLovDataRow2 = async () => {
      try {
        console.log('Fetching Row 2 LOV data...');
        ...
        try {
          ...
          console.error('Failed to get ID token for LOV Row 2:', tokenError);
          // Continue without token - API might still work or will return 401
        }
        ...
        const projectId = selectedProject?.id;
        const response = await fetch(
          `https://fuahu3jqsc.execute-api.ap-south-1.amazonaws.com/New/api/get/filter/resource-roster-lov-second?project_id=${projectId}`,
          {
            headers: headers
          }
        );
        ...
        console.log('Row 2 LOV API Response:', result);

        if (result.success && result.data) {
          console.log('Row 2 LOV Data received:', {
            processStream: result.data.ER_process_stream?.length || 0,
            application: result.data.ER_application?.length || 0,
            resourceLevel: result.data.ER_resource_level?.length || 0,
            onboardingStatus: result.data.ER_onboarding_status?.length || 0,
            billingStatus: result.data.BC_billability_status?.length || 0,
            workArrangement: result.data.SA_remote_onsite_hybrid?.length || 0
          });

          setLovData(prev => ({
            ...prev,
            processStream: result.data.ER_process_stream || [],
            application: result.data.ER_application || [],
            resourceLevel: result.data.ER_resource_level || [],
            onboardingStatus: result.data.ER_onboarding_status || [],
            billingStatus: result.data.BC_billability_status || [],
            workArrangement: result.data.SA_remote_onsite_hybrid || []
          }));
        } else {
          console.warn('Row 2 LOV API returned no data or success=false:', result);
        }
      } catch (error) {
        console.error('Error fetching Row 2 LOV data:', error);
        setLovError(error.message);
      }
    };
  ...
  // Fetch Level Definitions for mapping Level_Code to designation
  useEffect(() => {
    const fetchLevelDefinitions = async () => {
      try {
        ...
        try {
          ...
          console.error('Failed to get ID token for level definitions:', tokenError);
        }
    ...
    if (savedState) {
      try {
        ...
        if (!lovLoading && Object.keys(lovData).some(key => lovData[key].length > 0)) {
          ...
          const restoredData = dashboardState.filteredData || [];
          setFilteredData(restoredData.length > 0 ? sortDataByGrantAccess(restoredData) : []);
          setTableSearchText(dashboardState.tableSearchText || '');

          // Only restore LOV data if it's not already loaded
          if (Object.keys(dashboardState.lovData || {}).length > 0) {
            const restoredLovData = {};
            Object.keys(dashboardState.lovData).forEach(key => {
              if (dashboardState.lovData[key] && dashboardState.lovData[key].length > 0) {
                restoredLovData[key] = dashboardState.lovData[key];
              } else {
                restoredLovData[key] = lovData[key] || [];
              }
            });
            setLovData(restoredLovData);
          }

          // Check if data should be refreshed (returning from edit/create page)
          const shouldRefresh = dashboardState.shouldRefreshData;

          // Clear the saved state after restoring
          localStorage.removeItem('resourceDashboardState');

          // Refresh data from API if flag is set
          if (shouldRefresh) {
            // Use setTimeout to ensure state is updated before calling submitFilters
            setTimeout(() => {
              const refreshData = async () => {
                try {
                  const fieldMapping = {
                    organizationName: 'IC_organization_name',
                    businessLine: 'IC_business_line_name',
                    portfolioName: 'IC_portfolio_name',
                    serviceLineName: 'IC_service_line_name',
                    employmentType: 'IC_employment_type',
                    workLocation: 'IC_work_location',
                    nationality: 'IC_nationality',
                    processStream: 'ER_process_stream',
                    application: 'ER_application',
                    resourceLevel: 'ER_resource_level',
                    onboardingStatus: 'ER_onboarding_status',
                    billingStatus: 'BC_billability_status',
                    workArrangement: 'SA_remote_onsite_hybrid'
                  };

                  const queryParams = new URLSearchParams();
                  const restoredValues = dashboardState.selectedValues || {};

                  Object.entries(restoredValues).forEach(([key, values]) => {
                    if (values.length > 0) {
                  ...
                  const apiUrl = `https://fuahu3jqsc.execute-api.ap-south-1.amazonaws.com/New/api/get/filter/resource-roster-forms-filter?${queryParams.toString()}`;

                  let idToken = null;
                  try {
                    idToken = await getCachedToken();
                  } catch (tokenError) {
                    console.error('Failed to get ID token for refresh:', tokenError);
                  }
                  ...
                  if (data.success && data.data) {
                    setFilteredData(sortDataByGrantAccess(data.data));
                  }
  ...
  const handleViewDetails = (resourceId) => {
    ...
    navigate(`/dashboard/implementation-resource-form/edit/${resourceId}`);
  };
  ...
  const handleCreateNewForm = () => {
    ...
    navigate('/dashboard/implementation-resource-form/create');
  };
  ...
  const handleGrantAccess = () => {
    ...
    const executeGrantAccess = async () => {
      try {
        ...
        const resourceIds = selectedUsers.map(u => String(u.Resource_Roster_Form_id));
        const projectId = selectedProject?.id;
        ...
        const assignPromises = selectedUsers.map(user => {
          const payload = {
            email: user.IC_email,
            name: user.IC_full_name,
            project_id: projectId
          };

          return fetch('https://gl5xaesjob.execute-api.ap-south-1.amazonaws.com/New/api/admin/assign-user-to-project', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
        ...
        const hasAssignFailures = assignResults.some(res => res.success === false || res.error);
        if (hasAssignFailures) {
          setErrorMessage('Failed to update project access in the backend. Database field update aborted.');
          return;
        }
    ...
    const executeRevokeAccess = async () => {
      try {
        ...
        const removePromises = selectedUsers.map(user => {
          ...
          return fetch('https://gl5xaesjob.execute-api.ap-south-1.amazonaws.com/New/api/admin/remove-user-access', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
              'Authorization': `Bearer ${idToken}`
        ...
        const hasRemoveFailures = removeResults.some(res => res.success === false || res.error);
        if (hasRemoveFailures) {
          setErrorMessage('Failed to revoke project access in the backend. Database field update aborted.');
          return;
        }
  ...
  const submitFilters = async () => {
    ...
    try {
      ...
      try {
        ...
        console.error('Failed to get ID token for filter:', tokenError);
        // Continue without token - API might still work or will return 401
      }
  ...
  return (
    <div className="config-main" style={{ minHeight: '80vh', width: 'calc(98% - 2rem)', maxWidth: maxWidth, margin: '2rem auto', marginLeft: '2rem', marginRight: '2rem' }}>
      ...
      <div className="config-header" style={{ marginTop: '0', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <h2>Implementation Resource Dashboard</h2>
        <button
        ...
        <div style={{ marginBottom: '1rem' }}>
          <div style={{
            ...
            <div style={{ position: 'relative', width: '140px' }}>
              ...
              {openDropdown === 'resourceLevel' && (
                <div style={{
                  ...
                  {(lovData.resourceLevel || []).map((option, index) => (
                    <label
                      key={index}
                      data-dropdown-option="true"
                      style={{
                        ...
                        padding: '8px 10px',
                        cursor: 'pointer',
                        backgroundColor: selectedValues.resourceLevel.includes(option) ? '#e3f2fd' : 'white',
                        borderBottom: '1px solid #f0f0f0',
                        fontSize: '12px'
                      }}
                    >
                      <input
                        type="checkbox"
                        checked={selectedValues.resourceLevel.includes(option)}
                        onChange={() => toggleFilterValue('resourceLevel', option)}
                        style={{ marginRight: '8px', cursor: 'pointer' }}
                      />
                      {levelDefinitions.find(ld => ld.Level_Code === option)?.designation || option}
                    </label>
                  ))}
                </div>
              )}
            ...
            <div style={{ position: 'relative', width: '140px' }}>
              <button
                data-filter-button="true"
                onClick={() => toggleDropdown('billingStatus')}
                style={{
                  width: '100%',
                  padding: '6px 10px',
                  backgroundColor: selectedValues.billingStatus.length > 0 ? '#3b82f6' : '#f9f9f9',
                  color: selectedValues.billingStatus.length > 0 ? 'white' : '#333',
                  border: '1px solid #ddd',
                  borderRadius: '4px',
                  cursor: 'pointer',
                  fontSize: '13px',
              ...
              {openDropdown === 'billingStatus' && (
                <div style={{
                  ...
                  {(lovData.billingStatus || []).map((option, index) => (
                    <label
                      key={index}
                      data-dropdown-option="true"
                      style={{
                        ...
                        padding: '8px 10px',
                        cursor: 'pointer',
                        backgroundColor: selectedValues.billingStatus.includes(option) ? '#e3f2fd' : 'white',
                        borderBottom: '1px solid #f0f0f0',
                        fontSize: '12px'
                      }}
                    >
                      <input
                        type="checkbox"
                        checked={selectedValues.billingStatus.includes(option)}
                        onChange={() => toggleFilterValue('billingStatus', option)}
                        style={{ marginRight: '8px', cursor: 'pointer' }}
                      />
                      {option}
                    </label>
            ...
            <div style={{ position: 'relative', width: '140px' }}>
              <button
                data-filter-button="true"
                onClick={() => toggleDropdown('workArrangement')}
                style={{
                  width: '100%',
                  padding: '6px 10px',
                  backgroundColor: selectedValues.workArrangement.length > 0 ? '#3b82f6' : '#f9f9f9',
                  color: selectedValues.workArrangement.length > 0 ? 'white' : '#333',
                  border: '1px solid #ddd',
                  borderRadius: '4px',
                  cursor: 'pointer',
                  fontSize: '13px',
              ...
              {openDropdown === 'workArrangement' && (
                <div style={{
                  ...
                  {(lovData.workArrangement || []).map((option, index) => (
                    <label
                      key={index}
                      data-dropdown-option="true"
                      style={{
                        ...
                        padding: '8px 10px',
                        cursor: 'pointer',
                        backgroundColor: selectedValues.workArrangement.includes(option) ? '#e3f2fd' : 'white',
                        borderBottom: '1px solid #f0f0f0',
                        fontSize: '12px'
                      }}
                    >
                      <input
                        type="checkbox"
                        checked={selectedValues.workArrangement.includes(option)}
                        onChange={() => toggleFilterValue('workArrangement', option)}
                        style={{ marginRight: '8px', cursor: 'pointer' }}
                      />
                      {option}
                    </label>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ lineHeight: '1.2', whiteSpace: 'nowrap' }}>Grant Access</div>
            <input
              type="checkbox"
              ...
              onChange={(e) => {
                ...
                if (isChecked) {
                  const eligibleIndices = displayedData
                    .map((item, index) => ({ item, index }))
                    .filter(({ item }) => String(item.Grant_Access) !== "true" && String(item.save_Draft) !== "true")
                    .map(({ index }) => index);
          ...
          <div style={{
            ...
            <div style={{ lineHeight: '1.2', whiteSpace: 'nowrap' }}>Revoke Access</div>
            <input
              type="checkbox"
              ...
              title="Select All Revoke (Only granted records)"
            />
          </div>
          {/* <div style={{
            flex: 1,
        ...
        {displayedData.length > 0 ? (
          displayedData.map((item, index) => {
            ...
            const sanitizedFullName = DOMPurify.sanitize(item.IC_full_name || '', { ALLOWED_TAGS: [] });
            const sanitizedPrimaryRole = DOMPurify.sanitize(item.ER_primary_role || '', { ALLOWED_TAGS: [] });
            const sanitizedResourceLevel = DOMPurify.sanitize(item.ER_resource_level || '', { ALLOWED_TAGS: [] });
            const sanitizedProcessStream = DOMPurify.sanitize(item.ER_process_stream || '', { ALLOWED_TAGS: [] });
            const sanitizedOrgName = DOMPurify.sanitize(item.IC_organization_name || '', { ALLOWED_TAGS: [] });
            const sanitizedEmail = DOMPurify.sanitize(item.IC_email || '', { ALLOWED_TAGS: [] });
            return (
              <div
                key={item.Resource_Roster_Form_id || index}
                style={{
                  display: 'flex',
              ...
              >
                <div style={{
                  ...
                  <span
                    ...
                    onClick={() => handleViewDetails(item.Resource_Roster_Form_id || item.resource_form_id)}
                    onMouseEnter={(e) => e.target.style.textDecoration = 'underline'}
                    onMouseLeave={(e) => e.target.style.textDecoration = 'none'}
                  >
                    {sanitizedFullName || '-'}
                  </span>
                  {String(item.save_Draft) === "true" && (
                    <span style={{
                ...
                <div style={{
                  ...
                  {DOMPurify.sanitize(levelDefinitions.find(ld => ld.Level_Code === item.ER_resource_level)?.designation || sanitizedResourceLevel || '-', { ALLOWED_TAGS: [] })}
                </div>
                {/* <div style={{
                  flex: 1,
                  padding: '12px 12px',
                ...
                {/* <div style={{
                  ...
                  {item.ER_start_date ? new Date(item.ER_start_date).toLocaleDateString() : '-'}
                </div>
                <div style={{
                  flex: 1,
                  padding: '12px 12px',
                  ...
                  {item.ER_end_date ? new Date(item.ER_end_date).toLocaleDateString() : '-'}
                </div> */}
                {/* <div style={{
                  flex: 1,
                  padding: '12px 12px',
                ...
                <div style={{
                  ...
                  {item.BC_billing_start_date ? new Date(item.BC_billing_start_date).toLocaleDateString() : '-'}
                </div>
                <div style={{
                  flex: 1,
                  padding: '12px 12px',
                ...
                <div style={{
                  ...
                  padding: '12px 12px',
                  minWidth: '130px',
                  backgroundColor: (String(item.Grant_Access) === "true" || String(item.save_Draft) === "true") ? '#f8f8f8' : rowBgColor,
                  cursor: (String(item.Grant_Access) === "true" || String(item.save_Draft) === "true") ? 'not-allowed' : 'default',
                  display: 'flex',
                  justifyContent: 'center',
                  alignItems: 'center',
                  borderRight: '1px solid #ddd'
                  ...
                  <input
                    ...
                    disabled={String(item.Grant_Access) === "true" || String(item.save_Draft) === "true"}
                    onChange={(e) => {
                      if (e.target.checked) {
                        ...
                        const eligibleCount = displayedData.filter(d => String(d.Grant_Access) !== "true").length;
                        if (newSelected.length === eligibleCount && eligibleCount > 0) {
                          setSelectAllGrant(true);
                        }
                      } else {
                        const newSelected = selectedGrantRows.filter(i => i !== index);
                        setSelectedGrantRows(newSelected);
                        setSelectAllGrant(false);
                      }
                    }}
                    style={{
                      cursor: (String(item.Grant_Access) === "true" || String(item.save_Draft) === "true") ? 'not-allowed' : 'pointer',
                      width: '18px',
                      height: '18px'
                    }}
                    title={String(item.save_Draft) === "true" ? "Cannot grant access to draft records" : ""}
                  />
                </div>
                ...
                <div style={{
                  ...
                  padding: '12px 12px',
                  minWidth: '130px',
                  backgroundColor: (String(item.Grant_Access) !== "true" || String(item.save_Draft) === "true") ? '#f8f9fa' : rowBgColor,
                  cursor: (String(item.Grant_Access) !== "true" || String(item.save_Draft) === "true") ? 'not-allowed' : 'default',
                  display: 'flex',
                  justifyContent: 'center',
                  alignItems: 'center'
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      if (e.target.checked) {
                        ...
                        const eligibleCount = displayedData.filter(d => String(d.Grant_Access) === "true").length;
                        if (newSelected.length === eligibleCount && eligibleCount > 0) {
                          setSelectAllRevoke(true);
                        }
                      } else {
                        const newSelected = selectedRevokeRows.filter(i => i !== index);
                        setSelectedRevokeRows(newSelected);
                        setSelectAllRevoke(false);
                      }
                    }}
                    ...
                    title={String(item.save_Draft) === "true" ? "Cannot revoke access for draft records" : ""}
                  />
                </div>
                {/* <div style={{
                  flex: 1,
                  ...
                  {item.IC_primary_phone_no_code && item.IC_phone ? `${item.IC_primary_phone_no_code} ${item.IC_phone}` : '-'}
                </div> */}
              </div>
            );
          })
          ...
          <div style={{
            ...
            {filteredData.length === 0
              ? 'No filtered data available. Please apply filters and click "Submit" to see results.'
              : tableSearchText
              ...
              <div style={{ color: '#444', fontSize: '14px', lineHeight: '1.6' }}>
                ...
                <div style={{ marginBottom: '16px' }}>
                  ...
                  <p style={{ margin: '6px 0 0 0', color: '#555' }}>
                    The <strong>Implementation Resource Dashboard</strong> is a centralized view of all resources registered on the project. It allows administrators to filter, search, and manage system access for each resource by granting or revoking their project login.
                  </p>
                </div>
                ...
                <div style={{ marginBottom: '16px' }}>
                  ...
                  <ul style={{ margin: '6px 0 0 0', paddingLeft: '18px', color: '#555' }}>
                    <li><strong>Row 1</strong> — Organization Name, Business Line, Portfolio Name, Service Line Name, Employment Type, Work Location, Nationality. Click <strong>Apply Filters</strong> to fetch matching records.</li>
                    <li><strong>Row 2</strong> — Process Stream, Application, Resource Level, Onboarding Status, Billing Status, Work Arrangement. Click <strong>Apply Filters</strong> in Row 1 to apply all active selections.</li>
                ...
                <div style={{ marginBottom: '16px' }}>
                  ...
                  <ul style={{ margin: '6px 0 0 0', paddingLeft: '18px', color: '#555' }}>
                    ...
                    <li><strong>Resource Level</strong> — The resource&#39;s seniority level (mapped to a designation).</li>
...
export default NewDashbordResource;