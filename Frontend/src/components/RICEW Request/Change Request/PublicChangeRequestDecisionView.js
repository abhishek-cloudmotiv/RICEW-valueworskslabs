...
import { StandardAutocomplete } from './ChangeRequestFormAutocomplete';

const PublicChangeRequestDecisionView = () => {
    ...
    const fetchDetails = useCallback(async () => {
        try {
            ...
            // Reverting to GET as per your latest backend update
            const response = await changeRequestSubmitApiClient.get(`/publicView/ricew/ChangeRequest/getDetails?Change_Request_Form_id=${id}&Project_id=${projectId}`);
            ...
            if (response.success && response.data) {
                ...
                const sanitizedRecord = {
                    ...
                    Change_Request_Title: DOMPurify.sanitize(d.Change_Request_Title || '', { ALLOWED_TAGS: [] }),
                    ...
                    CR_Requestor_email: DOMPurify.sanitize(d.CR_Requestor_email || '', { ALLOWED_TAGS: [] }),
                    CR_Requestor_name: DOMPurify.sanitize(d.CR_Requestor_name || '', { ALLOWED_TAGS: [] }),
                    Change_Business_Owner_email: DOMPurify.sanitize(d.Change_Business_Owner_email || '', { ALLOWED_TAGS: [] }),
                    Change_Business_Owner_name: DOMPurify.sanitize(d.Change_Business_Owner_name || '', { ALLOWED_TAGS: [] }),
                    CR_Client_Owner_email: DOMPurify.sanitize(d.CR_Client_Owner_email || '', { ALLOWED_TAGS: [] }),
                    CR_Client_Owner_name: DOMPurify.sanitize(d.CR_Client_Owner_name || '', { ALLOWED_TAGS: [] }),
                ...
                // Pre-set decision if already exists
                setDecision(sanitizedRecord.Change_Approval_Status || '');
                setRejectionReason(sanitizedRecord.Change_Approval_Status === 'More Information Needed'
                    ? (sanitizedRecord.Request_Reason || sanitizedRecord.Change_Rejection_Reason || sanitizedRecord.Change_Rejected_Reason || '')
                    : (sanitizedRecord.Change_Rejected_Reason || sanitizedRecord.Change_Rejection_Reason || sanitizedRecord.Request_Reason || ''));
            } else {
            ...
            // For UI testing: We don't stop loading until a timeout if we want to show the loader
            // OR we just let the dummy data show.
    ...
    // Dynamic reason swap when decision changes manually
    React.useEffect(() => {
        if (!record) return;
        if (decision === 'More Information Needed') {
            const reason = record.Request_Reason || record.Change_Rejection_Reason || record.Change_Rejected_Reason || '';
            setRejectionReason(reason);
        } else if (decision === 'Rejected') {
            const reason = record.Change_Rejected_Reason || record.Change_Rejection_Reason || record.Request_Reason || '';
            setRejectionReason(reason);
        }
    }, [decision, record]);
    ...
    const handleSubmit = () => {
        if (!decision) {
            setStatusMessage('Please select a decision before submitting.');
            setShowError(true);
            setTimeout(() => setShowError(false), 5000);
            return;
        }
        if ((decision === 'Rejected' || decision === 'More Information Needed') && !rejectionReason.trim()) {
            setStatusMessage(`Please provide a ${decision === 'Rejected' ? 'rejection reason' : 'reason for more information'}.`);
            setShowError(true);
            setTimeout(() => setShowError(false), 5000);
            return;
        }
    ...
    const proceedWithSubmit = async () => {
        ...
        try {
            ...
            const payload = {
                ...
                Project_Name: DOMPurify.sanitize(record.Project_Name || 'N/A', { ALLOWED_TAGS: [] }),
                Change_Request_Title: DOMPurify.sanitize(record.Change_Request_Title || 'N/A', { ALLOWED_TAGS: [] }),
                Decision: DOMPurify.sanitize(decision || '', { ALLOWED_TAGS: [] }),
                Rejection_Reason: (decision === 'Rejected' || decision === 'More Information Needed') ? DOMPurify.sanitize(rejectionReason || '', { ALLOWED_TAGS: [] }) : '',
                CR_Requestor_email: DOMPurify.sanitize(record.CR_Requestor_email || '', { ALLOWED_TAGS: [] }),
                CR_Requestor_name: DOMPurify.sanitize(record.CR_Requestor_name || '', { ALLOWED_TAGS: [] }),
                Change_Business_Owner_email: DOMPurify.sanitize(record.Change_Business_Owner_email || '', { ALLOWED_TAGS: [] }),
                Change_Business_Owner_name: DOMPurify.sanitize(record.Change_Business_Owner_name || '', { ALLOWED_TAGS: [] }),
                CR_Client_Owner_email: DOMPurify.sanitize(record.CR_Client_Owner_email || '', { ALLOWED_TAGS: [] }),
                CR_Client_Owner_name: DOMPurify.sanitize(record.CR_Client_Owner_name || '', { ALLOWED_TAGS: [] }),
                updated_by: 'Public Stakeholder'
            };

            const response = await changeRequestSubmitApiClient.post('/email-Send/changeRequest/submitDecision', payload);

            if (response.success) {
                setStatusMessage('Decision submitted successfully!');
                setShowSuccess(true);
                setTimeout(() => setShowSuccess(false), 5000);
            } else {
                setStatusMessage(response.error || 'Failed to submit decision.');
                setShowError(true);
                setTimeout(() => setShowError(false), 5000);
            }
        } catch (error) {
    ...
    return (
        <div style={{ minHeight: '100vh', backgroundColor: '#f4f7fa', padding: '30px 20px', fontFamily: 'inherit' }}>
            <style>{`
                ...
                /* Overriding Autocomplete border for this specific view if needed */
                .decision-autocomplete input { border-color: #ddd !important; }
            `}</style>
            ...
            {/* Notifications */}
            {showSuccess && (
                <div style={{ position: 'fixed', top: '20px', right: '20px', backgroundColor: '#28a745', color: 'white', padding: '12px 24px', borderRadius: '4px', boxShadow: '0 4px 12px rgba(0,0,0,0.15)', zIndex: 3000, display: 'flex', alignItems: 'center', gap: '10px', fontSize: '14px', fontWeight: '500' }}>
            ...
            <div style={{ maxWidth: '1100px', minHeight: '650px', margin: '0 auto', backgroundColor: 'white', borderRadius: '8px', boxShadow: '0 10px 40px rgba(0,0,0,0.08)', overflow: 'hidden', border: '1px solid #ddd', display: 'flex', flexDirection: 'column' }}>
                ...
                <div style={{ padding: '20px 50px', flex: 1 }}>
                    {/* Page Title & Main File Link (Updated to Blue per request) */}
                    ...
                    {/* Section: Project Information & Details */}
                    ...
                    <div style={{ border: '1px solid #ddd', borderRadius: '4px', overflow: 'visible', marginBottom: (decision === 'Rejected' || decision === 'More Information Needed') ? '20px' : '50px' }}>
                        <div className="decision-autocomplete">
                            <StandardAutocomplete
                                value={decision}
                                onChange={(val) => setDecision(val)}
                                options={['Approved', 'More Information Needed', 'Rejected']}
                                label="Decision"
                                minWidth="220px"
                                placeholder="Select Decision"
                                showSearch={false}
                                style={{ borderRight: 'none', padding: '12px 24px' }}
                            />
                        </div>
                    </div>

                    {/* Reason / Rejection Reason (Conditional) */}
                    {(decision === 'Rejected' || decision === 'More Information Needed') && (
                        <div style={{ border: '1px solid #ddd', borderRadius: '4px', overflow: 'hidden', marginBottom: '50px', animation: 'fadeIn 0.3s ease' }}>
                            <div style={{
                                padding: '12px 24px',
                                background: '#f8f9fa',
                                ...
                                fontWeight: 'bold',
                                fontSize: '13px',
                                color: decision === 'Rejected' ? '#dc3545' : '#007bff',
                                display: 'flex',
                                alignItems: 'center',
                                gap: '8px'
                            }}>
                                ...
                                {decision === 'Rejected' ? 'Rejection Reason' : 'Details for More Information Needed'} <span style={{ color: '#dc3545' }}>*</span>
                            </div>
                            <textarea
                                value={rejectionReason}
                                onChange={(e) => setRejectionReason(e.target.value)}
                                placeholder={decision === 'Rejected' ? "Please explain why this request is being rejected..." : "Please specify what additional information is required..."}
                                style={{
                                    width: '100%',
                                    minHeight: '120px',
                                    padding: '15px 24px',
                    ...
                    <div style={{ display: 'flex', justifyContent: 'center', marginTop: 'auto' }}>
                        ...
                        >
                            {isSubmitting && <div style={{ width: '16px', height: '16px', border: '2px solid white', borderTop: '2px solid transparent', borderRadius: '50%', animation: 'spin 1s linear infinite' }}></div>}
                            Submit
                        </button>
                    </div>
                    ...
                    <div style={{
                        ...
                        <div style={{
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"></path><polyline points="22 4 12 14.01 9 11.01"></polyline></svg>
                                <h3 style={{ margin: 0, fontSize: '14px', fontWeight: 'bold', color: '#fff', letterSpacing: '0.5px' }}>Submit Decision</h3>
                            </div>
                        </div>
                        <div style={{ padding: '28px 24px 20px', color: '#333', fontSize: '14px', lineHeight: '1.7', textAlign: 'center', background: '#fff' }}>
                            <div style={{ marginBottom: '8px', color: '#555' }}>You are about to submit your final decision for this request.</div>
                            <div style={{ fontWeight: '600', color: '#333' }}>Are you sure you want to proceed with: <span style={{ color: decision === 'Approved' ? '#28a745' : decision === 'Rejected' ? '#dc3545' : '#007bff' }}>{decision}</span>?</div>
                        </div>
                        <div style={{
...
export default PublicChangeRequestDecisionView;
